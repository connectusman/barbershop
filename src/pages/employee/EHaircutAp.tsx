...
const EHaircutAp: React.FC <MyModalProps> = ({tempstr,shopM,custM,empM,bookM})  =>{
...
setLoading(true);
  ...
  var dres=await BookingModal.getbookingsbyceid(cr,empM.id,getDateTimeNumberS(d.toISOString()),getDateTimeNumberE(d.toISOString()))
  let len=dres.length;
  for(let i=0;i<len;i++)
  {
    var imgdata="";
  ...
  async function loaddata() {
    var allids=await BookingModal.getuniquecustomersofemp(empM.id);
