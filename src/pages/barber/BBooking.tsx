...
const BBooking: React.FC<MyModalProps> = ({tempstr,shopM,custM,empM,bookM}) =>{
  let hist=useHistory();
  ...
  d2.setMinutes(d2.getMinutes()-d2.getTimezoneOffset(),0,0);
    var res=await BookingModal.getBookingbyeid(empM.id,getDateTimeNumberS(d.toISOString()),getDateTimeNumberE(d.toISOString()),"false");
    var len=res.length;
    CBookings=[];
    for(var i=0;i<len;i++)
    {
  ...
  <IonPage>
    <IonHeader>
        ...
        <IonButtons slot="start">
          <IonBackButton text="" defaultHref="/bshophome"></IonBackButton>
        </IonButtons>
